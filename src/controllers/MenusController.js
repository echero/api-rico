...
module.exports = {
  ...
  delete: (req, res) => {
    ...
    if (idAlreadyInUse(id, Menus)) {
      ...
      console.log('trae error si el id del menu no es valida');
      res.status(404);
      res.json({ message: 'there are no menus with that id' });
    }
  },
  ...
  deletePlate: (req, res) => {
    ...
    const r = handlerMenu.deletePlate(id2, id);
    if (r) {
      res.status(201);
  ...
  deletePlates: (req, res) => {
    const id = parseInt(req.params.id, 10);
    const r = handlerMenu.deletePlates(id);
    if (r) {
      res.status(201);
