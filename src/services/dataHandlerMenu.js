...
const verifyAllPlates = (arr) => arr.every((e) => verifierDataPlate(e));
...
const modifyMenu = (menuModified) => {
  ...
  if (verifierDataMenu(menuModified)) {
    const menu = MenuById(menuModified.id);
    if (verifierIdPlates(menu.id, menuModified.plates)) menu.plates = menuModified.plates;
    done = true;
  } else {
    done = false;
  }
  ...
  if (typeof idM === 'number' && idAlreadyInUse(idM, Menus)) {
    ...
    if (menu.plates.length !== 0) {
      if (typeof id === 'number' && idAlreadyInUse(id, menu.plates)) {
        const plate = PlateById(id, idM);
        const index = menu.plates.indexOf(plate);
        if (index !== -1) {
  ...
  if (typeof id === 'number' && idAlreadyInUse(id, Menus)) {
    ...
    if (verifierDataPlate(plateModified)) {
      if (idAlreadyInUse(plateModified.id, menu.plates)) {
        const plate = PlateById(plateModified.id, menu.id);
